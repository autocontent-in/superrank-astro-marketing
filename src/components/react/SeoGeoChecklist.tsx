...
const SEO_ITEMS: ChecklistItem[] = [
  ...
  { id: '3', label: 'Canonical URL set', status: 'warn', suggestion: 'Add canonical tag to avoid duplicate content.' },
  { id: '4', label: 'Image alt text coverage', status: 'warn', suggestion: '3 images missing alt text.' },
  { id: '5', label: 'Internal linking structure', status: 'fail', suggestion: 'Add 5–10 internal links to related pages.' },
  { id: '6', label: 'Page load speed (LCP)', status: 'pass' },
  { id: '7', label: 'Mobile-friendly markup', status: 'pass' },
  { id: '8', label: 'Structured data (Schema)', status: 'warn', suggestion: 'Add Article or FAQ schema for rich snippets.' },
];

const GEO_ITEMS: ChecklistItem[] = [
  { id: '1', label: 'Generative engine visibility', status: 'warn', suggestion: 'Optimize for AI answer snippets.' },
  { id: '2', label: 'Clear, scannable content', status: 'pass' },
  { id: '3', label: 'Entity-focused headings', status: 'pass' },
  { id: '4', label: 'Factual, cited statements', status: 'warn', suggestion: 'Add 2–3 authoritative citations.' },
  { id: '5', label: 'Answer-first paragraphs', status: 'fail', suggestion: 'Lead with a direct answer in first 100 words.' },
  { id: '6', label: 'Structured Q&A format', status: 'warn', suggestion: 'Use FAQ blocks where relevant.' },
...
export default function SeoGeoChecklist() {
  ...
  const scoreDonutSeries = useMemo(() => [OVERALL_SCORE, 100 - OVERALL_SCORE], []);
  ...
  const categoryDonutSeries = useMemo(() => CATEGORY_SCORES.map((c) => c.value), []);
      ...
      <div className="border-b border-gray-200 bg-gray-50/80 px-6 py-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <span className="font-semibold text-gray-800">SEO & GEO Checklist</span>
      ...
      <div className="p-6">
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="flex flex-col items-center rounded-xl border border-gray-100 bg-gray-50/50 p-4">
            <p className="mb-2 text-sm font-medium text-gray-600">Overall Website Score</p>
          ...
          {/* Bar chart */}
          <div className="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
            <p className="mb-2 text-sm font-medium text-gray-600">Scores by Category</p>
            <Chart
              options={barOptions}
              series={barSeries}
              type="bar"
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2">
          <div>
            <h3 className="mb-3 flex items-center gap-2 text-base font-semibold text-gray-900">
              <span className="rounded bg-green-100 p-1.5 text-green-600">SEO</span>
            ...
            <ul className="space-y-2">
              {SEO_ITEMS.map((item) => (
                <li
                  key={item.id}
                  className="flex flex-wrap items-start justify-between gap-2 rounded-lg border border-gray-100 bg-white px-3 py-2.5 text-sm shadow-sm"
                >
                  <span className="text-gray-700">{item.label}</span>
                  <div className="flex flex-col items-end gap-1">
                    <StatusBadge status={item.status} />
                    {item.suggestion && (
                      <span className="max-w-[240px] text-right text-xs text-gray-500">
                        {item.suggestion}
                      </span>
                    )}
                  </div>
          ...
          <div>
            <h3 className="mb-3 flex items-center gap-2 text-base font-semibold text-gray-900">
              <span className="rounded bg-blue-100 p-1.5 text-blue-600">GEO</span>
              Suggestions &amp; improvements
            </h3>
            <ul className="space-y-2">
              {GEO_ITEMS.map((item) => (
